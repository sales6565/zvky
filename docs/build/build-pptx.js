...
module_({
  ...
  items: [
    ...
    'Private INSIDE chat: nobody reads a conversation they are not in through the panel, Super Admin included.',
    'OUTSIDE it there is one screen — Settings → Chat Activity — that reads everything, behind its own permission, held by the Super Admin alone. Reading it is itself recorded. Tell staff chat is logged.',
    'A new message also raises a DESKTOP notification, as an assignment does. Everyone is asked for browser permission; a refusal is reported in the app, not swallowed.',
  ],
