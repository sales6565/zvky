...
/* There was a generator here, temporaryPassword(), which produced a readable
 * one-off for an administrator's reset. It is gone because nothing calls it:
 * the studio asked for the reset to set one fixed value, the same one a new
 * account is created with, so the reset reads DEFAULT_PASSWORD in
 * src/routes/users.js instead. An exported generator nothing uses is a function
 * the next reader assumes is live. */
